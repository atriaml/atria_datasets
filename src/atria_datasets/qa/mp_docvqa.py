...
class _IMDBRecord:
    ...
    def from_raw(cls, record: dict[str, object]) -> _IMDBRecord:
        ...
        deduplicated_answers = list(dict.fromkeys(str(answer) for answer in raw_answers))
        ...
        return cls(
            ...
            ocr_tokens=[[str(token) for token in page_tokens] for page_tokens in ocr_tokens],
...
class MPDocVQA(Dataset[MultiPageDocumentInstance]):
    ...
    def _build_split_iterator(
        ...
        return SplitIterator(imdb_dir=Path(data_dir) / "mp_docvqa" / "imdb", split=split)
